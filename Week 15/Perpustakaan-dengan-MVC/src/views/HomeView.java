    ...
    private void initEvent(){
        btncariid.addActionListener(e -> {
            if(fieldinputktp.getText().length()!=0){
                MemberEntity member = new MemberController().getMemberEntity(fieldinputktp.getText());
                if(member!=null){
                    Viewable viewMember = new MemberController(member);
                    viewMember.view();
                    dispose();
                }else{
        ...
        btncaribuku.addActionListener(e -> {
            if(fieldcaribuku.getText().length()!=0){
                BookEntity book = new FoundBookController().getBook(fieldcaribuku.getText());
                if(book != null){
                    Viewable bookView = new FoundBookController(book);
                    bookView.view();
                }else{
        ...
        btnadmin.addActionListener(e -> {
            dispose();
            Viewable loginCashier = new LoginCashierController();
            loginCashier.view();
        });

        btnreg.addActionListener(e -> {
            Viewable register = new RegisterMemberController();
            register.view();
        });
